...
public class PlayerCollisionScript : MonoBehaviour {
	...
	void OnCollisionEnter(Collision col) 
	{
		...
		if (col.transform.gameObject.tag == "Projectile")
		{
			if (ChangeHPType == EnumChangeHPType.Normal)
			{
				gameObject.GetComponent<PlayerManager>().ChangeHP(col.gameObject.GetComponent<ProjectileScript>().Damage,col.contacts[0].point);
			}
			else if (ChangeHPType == EnumChangeHPType.CustomColor)
			{
				gameObject.GetComponent<PlayerManager>().ChangeHP(col.gameObject.GetComponent<ProjectileScript>().Damage, col.contacts[0].point,CustomColor);
			}
			else if (ChangeHPType == EnumChangeHPType.CustomForce)
			{
				gameObject.GetComponent<PlayerManager>().ChangeHP(col.gameObject.GetComponent<ProjectileScript>().Damage,col.contacts[0].point,CustomForce,CustomForceScatter);
			}
			else if (ChangeHPType == EnumChangeHPType.CustomColorandForce)
			{
				gameObject.GetComponent<PlayerManager>().ChangeHP(col.gameObject.GetComponent<ProjectileScript>().Damage,col.contacts[0].point,CustomForce,CustomForceScatter,CustomColor);
			}
			else if (ChangeHPType == EnumChangeHPType.CustomText)
			{
				gameObject.GetComponent<PlayerManager>().ChangeHP(col.gameObject.GetComponent<ProjectileScript>().Damage,col.contacts[0].point,"Custom Text");
            }
        }
